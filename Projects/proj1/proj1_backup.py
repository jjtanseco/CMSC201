...
# Description:
# This program allows users to play connect 4
# Collaboration:
...
def move(boardList, player1):
    ...
    while invalid:
        ...
        #if its within the number range: 
...
#################################################################################
###SOMEONE PLEASE CHECK IF THIS MAKES SENSE/WORKS PLEASE I'M SCARED THANK YOU ###
#################################################################################
...
# fakeMain():  NOT REAL MAIN >>> JUST FOR TESTING
def fakeMain():

    row = int(input("row length: "))

    col = int(input("col length: "))

    fakeBoard = createBoard(row, col)
...
# main():   Actual main, contains all other functions and runs whole program
# Input:    NONE
# Output:   The program?    
def main():
    ...
    #loops until they don't want to play anymore
    while replay == "y":
        ...
        boardList = createBoard(rowLength, colLength)
         ...
         #starts turn (prints the board then asks player to make move
        printBoard(boardList)
        ...
        #loops until winner is decided
        while winner == "none" or tie == False:
            ...
            #checks again for winner
            end = checkWinner(boardList, player1)
            #and for tie
            tie = checkTie(boardList)
